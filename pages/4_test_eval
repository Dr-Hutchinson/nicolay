...
def track_rerank_success(rerank_results: List[Dict], query: str, ideal_documents: List[str]) -> Tuple[int, float, float]:
    ...
    ranks = []
    for result in rerank_results:
       if result['UserQuery'] == query and str(result['TextID']) in ideal_documents:
...
if questions_df.empty:
  st.error("The 'benchmark_questions' sheet is empty or could not be loaded. Please check your data source.")
    ...
    selected_ideal_documents = selected_question_row['Ideal Documents'].split(',') if pd.notna(selected_question_row['Ideal Documents']) else []
    # Button to run RAG
