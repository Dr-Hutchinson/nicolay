...
def track_rerank_success(rerank_results: List[Dict], query: str, ideal_documents: List[str]) -> Tuple[int, float, float]:
    ...
    for result in rerank_results:
       if result['UserQuery'] == query and str(result['text_id']) in ideal_documents:
...
def test_track_rerank_success():
  test_rerank_results = [
      {'UserQuery': 'test_query', 'Rank': 1, 'text_id': '100', 'Relevance Score': 0.95},
      {'UserQuery': 'test_query', 'Rank': 2, 'text_id': '200', 'Relevance Score': 0.85},
      {'UserQuery': 'test_query', 'Rank': 3, 'text_id': '300', 'Relevance Score': 0.75},
      {'UserQuery': 'test_query', 'Rank': 4, 'text_id': '400', 'Relevance Score': 0.65}
  ]
  ...
  st.write(f"Test case: hits:{hits}, precision: {precision}, avg_rank: {average_rank}")
